...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        """
        Возвращает кортеж из пяти элементов: (anchor, positive, negative, anchor_label, negative_label).
        Anchor и positive принадлежат одному классу, negative – из другого.
        """
...
# Функция для одной эпохи обучения с semi hard negative mining
def train_one_epoch(model, dataloader, optimizer, device, margin=1.0, semi_hard=True):
    model.train()  # Переводим модель в режим обучения
    running_loss = 0.0

    for batch_idx, batch in enumerate(dataloader):
        # Ожидаем, что батч состоит из 5 элементов: anchor, positive, negative, anchor_label, negative_label
        if len(batch) == 5:
            anchor, positive, negative, anchor_label, negative_label = batch
        else:
            # Если метки отсутствуют, используем стандартный подход
        ...
        # Метки приводим к устройству (используем для negative mining)
        ...
        if semi_hard:
            # Формируем кандидатный пул из эмбеддингов anchor и negative
            ...
            for i in range(batch_size):
                # Вычисляем расстояние между anchor и positive
                d_ap = torch.norm(anchor_out[i] - positive_out[i], p=2)

                # Выбираем кандидатов, чей класс отличается от класса anchor
                mask = (candidate_labels != anchor_label[i])
                if mask.sum() == 0:
                    chosen_negative = negative_out[i]
                else:
                    candidate_emb = candidate_embeddings[mask]
                    # Вычисляем расстояния от anchor[i] до кандидатов
                    d_an = torch.norm(anchor_out[i].unsqueeze(0) - candidate_emb, p=2, dim=1)
                    # Выбираем те, для которых выполняется условие semi hard:
                    # d(a, p) < d(a, n) < d(a, p) + margin
                    semi_hard_mask = (d_an > d_ap) & (d_an < d_ap + margin)
                    if semi_hard_mask.sum() > 0:
                        # Выбираем кандидата с минимальным расстоянием из semi hard отрицательных
                        ...
                        # Если подходящих кандидатов нет, используем предоставленный негатив
                        chosen_negative = negative_out[i]
                # Вычисляем triplet loss для i-го примера
                d_an_final = torch.norm(anchor_out[i] - chosen_negative, p=2)
                loss_i = torch.relu(d_ap - d_an_final + margin)
                batch_loss += loss_i
            loss = batch_loss / batch_size
        else:
            # Стандартный подход с использованием предоставленных негативов
...
# (исключая само изображение). Если среди них найден хотя бы один с тем же классом, считается попадание.
...
def main():
    ...
    # Определяем оптимизатор и используем стандартную функцию потерь для валидации
